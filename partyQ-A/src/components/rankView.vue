...
const tabs = ref([
    { id: 'Tab1', name: '当前排行榜', content: { num: 1, username: '用户名', rightPropotion: '12', time: '10:00' } },
    { id: 'Tab2', name: '往期排行榜', content: { num: 2, username: '用户名', rightPropotion: '23', time: '10:00' } },
]);
...
<template>
    ...
    <div class="mainContect">
        ...
        <div v-for="(tab, index) in tabs" :key="index" :class="['data', { active: activeTab === tab.id }]">
            <div style="color: rgba(95, 91, 91, 1);opacity: 0.4;">{{ tab.content.num }}</div>
            <div>{{ tab.content.username }}</div>
            <div style="color: rgba(95, 91, 91, 1);">{{ tab.content.rightPropotion }}</div>
